...
class SpUser extends Model {
	...
	public function createDefaultUser() {
		if (! $this->isUser ( "--" )) {
			...
			$sql = "INSERT INTO sp_users (login, firstname, name, id_status, pwd, id_unit,
	...
	public function isActive($login){
		$sql = "select id, is_active from sp_users where login=?";
	...
	public function connect($login, $pwd) {
		$sql = "select id, is_active from sp_users where login=? and pwd=?";
		$user = $this->runRequest ( $sql, array (
				$login,
				md5 ( $pwd ) 
		) );
	...
	public function updateLastConnection($userId) {
		$sql = "update sp_users set date_last_login=? where id=?";
	...
	public function getUser($login, $pwd) {
		$sql = "select id as idUser, login as login, pwd as pwd, id_status, is_active 
            from sp_users where login=? and pwd=?";
		$user = $this->runRequest ( $sql, array (
				$login,
				md5 ( $pwd ) 
		) );
	...
	public function userLogin($id){
		$sql = "select login from sp_users where id=?";
		$user = $this->runRequest ( $sql, array (
				$id
		) );
		if ($user->rowCount () == 1){
	...
	public function getUsers($sortentry = 'id') {
		$sql = "select * from sp_users order by " . $sortentry . " ASC;";
	...
	public function getAllActifEmails(){
		$sql = "select email from sp_users where is_active=1 order by name ASC;";
		$user = $this->runRequest ( $sql );
		return $user->fetchAll ();
	}
	...
	public function getActiveUsers($sortentry = 'id', $is_active = 1) {
		$sql = "select * from sp_users where is_active=" . $is_active . " order by " . $sortentry . " ASC;";
		$user = $this->runRequest ( $sql );
		return $user->fetchAll ();
	}
	...
	public function getUsersSummary($sortentry = 'id', $active = 1) {
		$sql = "select id, name, firstname from sp_users where is_active >= " . $active . " order by " . $sortentry . " ASC;";
		$user = $this->runRequest ( $sql );
		return $user->fetchAll ();
	}
	...
	public function getUserUnit($id){
		$sql = "select id_unit from sp_users where id=?";
		$user = $this->runRequest ( $sql, array ($id) );
		$userf = $user->fetch ();
		return $userf [0];
	}
	...
	public function getUserEmail($id){
		$sql = "select email from sp_users where id=?";
		$user = $this->runRequest ( $sql, array ($id) );
		$userf = $user->fetch ();
		return $userf [0];
	}
	...
	public function getActiveUsersInfo($sortentry = 'id', $is_active = 1) {
		...
		$sql = "SELECT user.id AS id, user.login AS login, 
    				   ...
    				   sp_units.name AS unit, 
    				   resp.name AS resp_name, resp.firstname AS resp_firstname,
    				   core_status.name AS status
    			FROM sp_users AS user
    			INNER JOIN sp_users AS resp ON user.id_responsible = resp.id
    			INNER JOIN sp_units ON user.id_unit = sp_units.id
	...
	public function getUsersUnitInfoSearch($selectEntry, $searchTxt, $sortentry = ""){
		...
		$sql = "SELECT user.id AS id, user.login AS login,
    			...
    			INNER JOIN sp_units ON user.id_unit = sp_units.id
    			INNER JOIN core_status ON user.id_status = core_status.id
    			WHERE sp_units.".$selectEntry." LIKE '%$searchTxt%'";
	...
	public function userAllInfo($id) {
		$sql = "select * from sp_users where id=?";
		$user = $this->runRequest ( $sql, array (
				$id 
		) );
		$userquery = null;
	...
	public function addUser($name, $firstname, $login, $pwd, $email, $phone, $id_unit, $id_responsible, $id_status, $convention, 
		...
		$sql = "insert into sp_users(login, firstname, name, email, tel, pwd, id_unit, id_responsible, 
	...
	public function updateUser($id, $firstname, $name, $login, $email, $phone, $id_unit, $id_responsible, $id_status, $convention, 
							   $date_convention, $date_end_contract = "", $is_active = 1, $source = "local") {
		$sql = "update sp_users set login=?, firstname=?, name=?, email=?, tel=?, id_unit=?, id_responsible=?, id_status=?,
	...
	public function setExtBasicInfo($login, $name, $firstname, $email, $id_status){
		...
		// insert
		if (! $this->isUser ( $login )){
			$sql = "insert into sp_users(login, firstname, name, email, id_status, source, date_created, id_unit, id_responsible)" . " values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$this->runRequest ( $sql, array (
					$login,
					$firstname,
					$name,
		...
		// update
			...
			$sql = "update sp_users set firstname=?, name=?, email=?
    			                  where login=?";
			$this->runRequest ( $sql, array (
					$firstname,
					$name,
	...
	public function changePwd($id, $pwd) {
		$sql = "update sp_users set pwd=? where id=?";
		$user = $this->runRequest ( $sql, array (
				md5 ( $pwd ),
				$id 
		) );
	...
	public function getpwd($id) {
		$sql = "select pwd from sp_users where id=?";
		$user = $this->runRequest ( $sql, array (
				$id 
		) );
		$userquery = null;
	...
	public function updateUserAccount($id, $firstname, $name, $email, $phone) {
		$sql = "update sp_users set firstname=?, name=?, email=?, tel=? where id=?";
		$this->runRequest ( $sql, array (
				$firstname,
				$name,
				$email,
	...
	public function isUser($login) {
		$sql = "select id from sp_users where login=?";
		$user = $this->runRequest ( $sql, array (
				$login 
		) );
		if ($user->rowCount () == 1)
	...
	public function isLocalUser($login) {
		$sql = "select id from sp_users where login=? AND source=?";
		$user = $this->runRequest ( $sql, array (
				$login, "local"
		) );
		if ($user->rowCount () == 1)
	...
	public function setUnitId($login, $unitId) {
		$sql = "update sp_users set id_unit=? where login=?";
		$this->runRequest ( $sql, array (
				$unitId,
				$login 
		) );
	...
	public function getUserIdFromFullName($fullname) {
		$sql = "select firstname, name, id from sp_users";
		$user = $this->runRequest ( $sql );
		$users = $user->fetchAll ();
	...
	public function setResponsible($idUser, $idResp) {
		$sql = "update sp_users set id_responsible=? where id=?";
		$this->runRequest ( $sql, array (
				$idResp,
				$idUser 
		) );
	...
	public function getResponsibleOfUnit($unitId) {
		$sql = "select id, name, firstname from sp_users where id in (select id_users from sp_responsibles) and id_unit=? ORDER BY name";
	...
	public function getUserFromlup($id, $unit_id, $responsible_id) {
		$sql = 'SELECT name, firstname, id_responsible FROM sp_users WHERE id=? AND id_unit=? AND id_responsible=?';
	...
	public function getUserFromIdUnit($id, $unit_id) {
		$sql = "SELECT name, firstname, id_responsible, id_unit FROM sp_users WHERE id=?";
		if ($unit_id > 0) {
	...
	public function getLastConnection($id) {
		$sql = "select date_last_login from sp_users where id=?";
		$user = $this->runRequest ( $sql, array (
				$id 
		) );
		if ($user->rowCount () == 1) {
	...
	public function setactive($id, $active) {
		$sql = "update sp_users set is_active=? where id=?";
		$this->runRequest ( $sql, array (
				$active,
				$id 
		) );
	...
	private function updateUserActiveContract() {
		$sql = "select id, date_end_contract from sp_users where is_active=1";
		$req = $this->runRequest ( $sql );
		$users = $req->fetchAll ();
	...
	private function updateUserActiveLastLogin($numberYear) {
		$sql = "select id, date_last_login from sp_users where is_active=1";
		$req = $this->runRequest ( $sql );
		$users = $req->fetchAll ();
	...
	public function activate($userId) {
		...
		$sql = "update sp_users set is_active=?, date_last_login=? where id=?";
		$this->runRequest ( $sql, array (
				1,
				$today,
				$userId 
	...
	public function getUserFromNameFirstname($name, $firstname) {
		$sql = "select id from sp_users where name=? and firstname=?";
		$user = $this->runRequest ( $sql, array (
				$name,
				$firstname 
		) );
	...
	public function getConventionList() {
		$sql = "select distinct convention from sp_users order by convention";
		$req = $this->runRequest ( $sql );
		return $req->fetchAll ();
	}
	/**
	...
	public function getUserConvention($id) {
		$sql = "select convention from sp_users where id=?";
		$req = $this->runRequest ( $sql, array (
				$id 
		) );
		$tmp = $req->fetch ();
	...
	public function exportResponsible($idType) {
		...
		if ($idType == 0) { // all
			$sql = "SELECT * FROM sp_users WHERE id IN (SELECT id_users FROM sp_responsibles) ORDER BY name ASC";
			$req = $this->runRequest ( $sql );
			$resps = $req->fetchAll ();
		} else if ($idType == 1) { // active
			$sql = "SELECT * FROM sp_users WHERE id IN (SELECT id_users FROM sp_responsibles) AND is_active=? ORDER BY name ASC";
			$req = $this->runRequest ( $sql, array(1));
			$resps = $req->fetchAll ();
			$typeResp = "actifs";
		} else if ($idType == 2) { // inactive
			$sql = "SELECT * FROM sp_users WHERE id IN (SELECT id_users FROM sp_responsibles) AND is_active=0 ORDER BY name ASC";
	...
	public function findFromAC($resp){
		...
		if ( count($respArray) > 1){
			...
			$sql = "select id from sp_users where name=?";
			$user = $this->runRequest ( $sql, array (
					$name
			) );
	...
	public function delete($id){
		$sql="DELETE FROM sp_users WHERE id = ?";
		$req = $this->runRequest($sql, array($id));
	}
	public function setEndContract($login, $date_fin_contrat){
		$sql = "update sp_users set date_end_contract=? where login=?";
		$this->runRequest ( $sql, array (
				$date_fin_contrat,
				$login
		) );
