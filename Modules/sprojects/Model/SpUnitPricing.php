 ...
 */
class SpUnitPricing extends Model {
	...
	public function allPricing(){
		$sql = "select * from sp_unitpricing";
	...
	public function allPricingTable(){
		...
		for($i = 0 ; $i < count($pricingsIds) ; ++$i){
			...
			if ($supliesusersdatabase == "local"){
				$sql = "select id, name from sp_units where id=?";
			...
			// get pricing info
			$sql = "select id, tarif_name from sp_pricing where id=?";
	...
	{
		$sql = "select id_pricing from sp_unitpricing where id_unit=?";
	...
	public function editPricing($id_unit, $id_pricing){
		$sql = "update sp_unitpricing set id_pricing=? where id_unit=?";
		$this->runRequest($sql, array($id_pricing, $id_unit));
	}
	
	public function addPricing($id_unit, $id_pricing){
		$sql = "insert into sp_unitpricing(id_unit, id_pricing)"
				. " values(?, ?)";
