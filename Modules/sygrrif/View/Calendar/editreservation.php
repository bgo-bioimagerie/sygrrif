...
<?php Require_once 'Modules/projet/Model/ResTranslator.php'?>
<?php
$readOnlyGlobal = ""; 
...
$(document).ready(function() {
    ...
    $('#acronyme2').hide(); // on cache le champ par défaut
     
    $('select[name="acronyme"]').change(function() { // lorsqu'on change de valeur dans la liste
...
$(document).ready(function() {
    ...
    $('#visite2').hide(); // on cache le champ par défaut
     
    $('select[name="numerovisite"]').change(function() { // lorsqu'on change de valeur dans la liste
    var valeur = $(this).val(); // valeur sélectionnée
...
<div class="container">
	...
	<form role="form" class="form-horizontal" action="calendar/editreservationquery" method="post">
		...
		<div class="form-group">
			...
			<?php 
				...
				if(isset($reservationInfo))
				{
					$numerovisite = $reservationInfo["numerovisite"];
				}
				$reservations= $donnee;
				if(isset($reservationInfo) && $allowedBookForOther==TRUE)
				{?>
		...
		<div class="form-group">
				<?php 
				if (isset($reservationInfo)){
					$stime = intval($this->clean($reservationInfo['start_time']));
					echo $stime;
      ...
      ?>
		...
		<?php 
			...
			?>
		<div class="form-group">
			<label for="inputEmail" class="control-label col-xs-4"><?=SyTranslator::Short_desc($lang)?></label>
			<div class="col-xs-8">
