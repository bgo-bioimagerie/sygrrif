...
class ControllerSygrrifpricing extends ControllerSecureNav {
    ...
    public function generateAllBills($searchDate_starti, $searchDate_endi) {
        ...
        if ($searchDate_start != "" && $searchDate_end != "") {
            ...
            foreach ($unitsListTmp as $unit) {
                if ($modelCalEntry->hasUnitEntry($unit["id"], $startdate, $enddate) == true) {
                    ...
                    $this->appendLog("No reservation found for unit " . $unit["name"], $logFile);
                }
            }
        }
        ...
        for ($u = 0; $u < count($unitsList); $u++) {
            ...
            for ($r = 0; $r < count($responsiblesList); $r++) {
                ...
                $message = $billgenaratorModel->generateBill($searchDate_start, $searchDate_end, $unitsList[$u]['id'], $responsiblesList[$r]['id'], $billDir, $noBill);
                $this->appendLog($message, $logFile);
            }
        }
    ...
    protected function appendLog($txt, $fileUrl) {
        $content = "";
