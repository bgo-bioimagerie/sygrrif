...
abstract class ControllerBooking extends ControllerSecureNav {
    ...
    public function calendarMenuData($curentSiteId, $curentAreaId, $curentResourceId, $curentDate) {
        ...
        if (array_search($curentAreaId, $areas) == false){
            $curentAreaId = $areas[0]["id"];
        }
        //echo "curentAreaId = " . print_r($curentAreaId) . "<br/>";
    ...
    protected function hasAuthorization($id_resourcecategory, $resourceAccess, $id_user, $userStatus, $curentDateUnix) {
        ...
        // get the resource site
        $modelResCat = new SyResourcesCategory();
        $id_site = $modelResCat->getSite($id_resourcecategory);
        
        $is_userSiteManager = true;
        if ($id_site > 1) {
            $modelSite = new CoreSite();
            $is_userSiteManager = $modelSite->isUserSiteManager($id_user, $id_site);
        }
        ...
        if ($resourceAccess == 2) {
            //echo "pass 1 </Br>";
            if ($userStatus > 2 && $is_userSiteManager) {
                $isUserAuthorizedToBook = true;
            }
            if ($userStatus == 2) {
                //echo "pass </Br>";
        ...
        if ($resourceAccess == 3) {
            if ($userStatus >= 3 && $is_userSiteManager) {
                $isUserAuthorizedToBook = true;
            }
        }
        if ($resourceAccess == 4 && $is_userSiteManager) {
            if ($userStatus >= 4) {
                $isUserAuthorizedToBook = true;
            }
        }
