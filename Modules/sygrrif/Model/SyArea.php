...
class SyArea extends Model {
    ...
    public function createTable() {
        ...
        $this->addColumn("sy_areas", "id_site", "int(11)", 1);
    }
     ...
     */
    public function getUnrestrictedAreasIDName($id_site = "") {
        if ($id_site == ""){
            $sql = "select id, name from sy_areas where restricted=0;";
            $data = $this->runRequest($sql);
        }
        else{
            $sql = "SELECT id, name FROM sy_areas WHERE id_site=? AND restricted=0;";
     ...
     * @return multitype: Areas info
     */
    public function getAreasIDName($id_site = "") {
        if ($id_site == ""){
            $sql = "select id, name from sy_areas;";
            $data = $this->runRequest($sql);
        }
        else{
            $sql = "SELECT id, name FROM sy_areas WHERE id_site=?;";
            $data = $this->runRequest($sql, array($id_site));
        }
        return $data->fetchAll();
    }
     ...
     */
    public function addArea($name, $display_order, $restricted, $id_site = 1) {

        $sql = "insert into sy_areas(name, display_order, restricted, id_site)"
                . " values(?,?,?,?)";
        $this->runRequest($sql, array($name, $display_order, $restricted, $id_site));
     ...
     */
    public function importArea($id, $name, $display_order, $restricted, $id_site = 1) {

        if ($this->isAreaId($id)) {
            $this->updateArea($id, $name, $display_order, $restricted, $id_site);
        } else {
            $sql = "insert into sy_areas(id, name, display_order, restricted, id_site)"
                    . " values(?,?,?,?,?)";
            $this->runRequest($sql, array($id, $name, $display_order, $restricted));
        }
    }
     ...
     * @param number $restricted
     */
    public function setArea($name, $display_order, $restricted, $id_site = 1) {
        if (!$this->isArea($name)) {
            $this->addArea($name, $display_order, $restricted, $id_site);
        }
    }
     ...
     */
    public function updateArea($id, $name, $display_order, $restricted, $id_site = 1) {
        $sql = "update sy_areas set name= ?, display_order=?, restricted=?, id_site=?
									  where id=?";
        $this->runRequest($sql, array($name, $display_order, $restricted, $id_site, $id));
    }
