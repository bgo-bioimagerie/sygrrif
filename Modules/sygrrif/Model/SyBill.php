...
class SyBill extends Model {
    ...
    public function createTable() {
        ...
        $this->addColumn("sy_bills", "total_ht", "float(11)", 0);
        $this->addColumn("sy_bills", "id_site", "int(11)", 1);
    }
     ...
     */
    public function addBill($number, $date_generated, $date_paid = "", $is_paid = 0, $id_site = 1) {

        $sql = "insert into sy_bills(number, date_generated, date_paid, is_paid, id_site)"
                . " values(?, ?, ?, ?, ?)";
        $this->runRequest($sql, array($number, $date_generated, $date_paid, $is_paid, $id_site));
     ...
     */
    public function addBillUnit($number, $period_begin, $period_end, $date_generated, $id_unit, $id_responsible, $totalHT, $date_paid = "", $is_paid = 0, $id_site = 1) {
        $sql = "insert into sy_bills(number, period_begin, period_end, date_generated, id_unit, id_responsible, total_ht, date_paid, is_paid, id_site)"
                . " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $this->runRequest($sql, array($number, $period_begin, $period_end, $date_generated, $id_unit, $id_responsible, $totalHT, $date_paid, $is_paid, $id_site));
        return $this->getDatabase()->lastInsertId();
    }
    ...
    public function getBillsForManager($id_user, $sortentry = 'id'){
        $sql = "SELECT * FROM sy_bills WHERE id_site IN (SELECT id_site FROM core_j_user_site WHERE id_user=? AND id_status>=3) order by " . $sortentry . " ASC;";
    ...
    public function getBillsUnit($sortentry = 'id') {
        ...
        public function getBillsUnitForManager($id_user, $sortentry = 'id') {
        $sqlSort = "sy_bills.id";
        if ($sortentry == "number") {
            $sqlSort = "sy_bills.number";
        } else if ($sortentry == "date_generated") {
        ...
        $sql = "SELECT sy_bills.id AS id, sy_bills.number AS number,
					   ...
					   core_users.firstname AS resp_firstname
				FROM sy_bills
                                WHERE sy_bills.id_site IN (SELECT id_site FROM core_j_user_site WHERE id_user=? AND id_status>=3) 
				INNER JOIN core_units ON sy_bills.id_unit = core_units.id
				INNER JOIN core_users ON sy_bills.id_responsible = core_users.id	
				ORDER BY " . $sqlSort . ";";
     ...
     */
    public function editBills($id, $number, $date_generated, $total_ht, $date_paid, $is_paid, $id_site) {

        $sql = "update sy_bills set number=?, date_generated=?, total_ht=?, date_paid=?, is_paid=?, id_site=?  where id=?";
        $this->runRequest($sql, array($number, $date_generated, $total_ht, $date_paid, $is_paid, $id_site, $id));
    }
     ...
     * @param number $is_paid
     */
    public function editBillUnit($id, $number, $date_generated, $id_unit, $id_responsible, $date_paid, $is_paid, $id_site) {

        $sql = "update sy_bills set number=?, date_generated=?, id_unit=?, id_responsible=?, date_paid=?, is_paid=?, id_site=?  where id=?";
        $this->runRequest($sql, array($number, $date_generated, $id_unit, $id_responsible, $date_paid, $is_paid, $id_site, $id));
    }
    ...
    public function editBillProject($id, $number, $date_generated, $id_project, $date_paid, $is_paid) {
        ...
        $this->runRequest($sql, array($number, $date_generated, $id_project, $date_paid, $is_paid, $id));
    }
