...
class SyAuthorization extends Model {
    ...
    public function getActiveAuthorizationsForManager($id_user, $is_active = 1) {
       
        $sql = "SELECT * from sy_authorization WHERE resource_id IN (SELECT id FROM sy_resourcescategory WHERE id_site IN (SELECT id_site FROM core_j_user_site WHERE id_user=? AND id_status>=3)) AND is_active=" . $is_active . ";";
        $auth = $this->runRequest($sql, array($id_user, $is_active));
        return $auth->fetchAll();
    }
