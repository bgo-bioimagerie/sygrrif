...
class SuUser extends Model {
	...
	public function createDefaultUser() {
		if (! $this->isUser ( "--" )) {
			...
			$sql = "INSERT INTO su_users (login, firstname, name, id_status, pwd, id_unit,
	...
	public function isActive($login){
		$sql = "select id, is_active from su_users where login=?";
	...
	public function connect($login, $pwd) {
		$sql = "select id, is_active from su_users where login=? and pwd=?";
		$user = $this->runRequest ( $sql, array (
				$login,
				md5 ( $pwd ) 
		) );
	...
	public function updateLastConnection($userId) {
		$sql = "update su_users set date_last_login=? where id=?";
	...
	public function getUser($login, $pwd) {
		$sql = "select id as idUser, login as login, pwd as pwd, id_status, is_active 
            from su_users where login=? and pwd=?";
		$user = $this->runRequest ( $sql, array (
				$login,
				md5 ( $pwd ) 
		) );
	...
	public function userLogin($id){
		$sql = "select login from su_users where id=?";
		$user = $this->runRequest ( $sql, array (
				$id
		) );
		if ($user->rowCount () == 1){
	...
	public function getAllActifEmails(){
		$sql = "select email from su_users where is_active=1 order by name ASC;";
		$user = $this->runRequest ( $sql );
		return $user->fetchAll ();
	}
	...
	public function getActiveUsers($sortentry = 'id', $is_active = 1) {
		$sql = "select * from su_users where is_active=" . $is_active . " order by " . $sortentry . " ASC;";
		$user = $this->runRequest ( $sql );
		return $user->fetchAll ();
	}
	...
	public function getUserUnit($id){
		$sql = "select id_unit from su_users where id=?";
		$user = $this->runRequest ( $sql, array ($id) );
		$userf = $user->fetch ();
		return $userf [0];
	}
	...
	public function getUserEmail($id){
		$sql = "select email from su_users where id=?";
		$user = $this->runRequest ( $sql, array ($id) );
		$userf = $user->fetch ();
		return $userf [0];
	}
	...
	public function getActiveUsersInfo($sortentry = 'id', $is_active = 1) {
		...
		$sql = "SELECT user.id AS id, user.login AS login, 
    				   ...
    				   su_units.name AS unit, 
    				   resp.name AS resp_name, resp.firstname AS resp_firstname,
    				   core_status.name AS status
    			FROM su_users AS user
    			INNER JOIN su_users AS resp ON user.id_responsible = resp.id
    			INNER JOIN su_units ON user.id_unit = su_units.id
	...
	public function getUsersUnitInfoSearch($selectEntry, $searchTxt, $sortentry = ""){
		...
		$sql = "SELECT user.id AS id, user.login AS login,
    			...
    			INNER JOIN su_units ON user.id_unit = su_units.id
    			INNER JOIN core_status ON user.id_status = core_status.id
    			WHERE su_units.".$selectEntry." LIKE '%$searchTxt%'";
	...
	public function addUser($name, $firstname, $login, $pwd, $email, $phone, $id_unit, $id_responsible, $id_status, $convention, 
		...
		$sql = "insert into su_users(login, firstname, name, email, tel, pwd, id_unit, id_responsible, 
	...
	public function updateUser($id, $firstname, $name, $login, $email, $phone, $id_unit, $id_responsible, $id_status, $convention, 
							   $date_convention, $date_end_contract = "", $is_active = 1, $source = "local") {
		$sql = "update su_users set login=?, firstname=?, name=?, email=?, tel=?, id_unit=?, id_responsible=?, id_status=?,
	...
	public function setExtBasicInfo($login, $name, $firstname, $email, $id_status){
		...
		// insert
		if (! $this->isUser ( $login )){
			$sql = "insert into su_users(login, firstname, name, email, id_status, source, date_created, id_unit, id_responsible)" . " values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$this->runRequest ( $sql, array (
					$login,
					$firstname,
					$name,
		...
		// update
			...
			$sql = "update su_users set firstname=?, name=?, email=?
    			                  where login=?";
			$this->runRequest ( $sql, array (
					$firstname,
					$name,
	...
	public function changePwd($id, $pwd) {
		$sql = "update su_users set pwd=? where id=?";
		$user = $this->runRequest ( $sql, array (
				md5 ( $pwd ),
				$id 
		) );
	...
	public function getpwd($id) {
		$sql = "select pwd from su_users where id=?";
		$user = $this->runRequest ( $sql, array (
				$id 
		) );
		$userquery = null;
	...
	public function updateUserAccount($id, $firstname, $name, $email, $phone) {
		$sql = "update su_users set firstname=?, name=?, email=?, tel=? where id=?";
		$this->runRequest ( $sql, array (
				$firstname,
				$name,
				$email,
	...
	public function isUser($login) {
		$sql = "select id from su_users where login=?";
		$user = $this->runRequest ( $sql, array (
				$login 
		) );
		if ($user->rowCount () == 1)
	...
	public function isLocalUser($login) {
		$sql = "select id from su_users where login=? AND source=?";
		$user = $this->runRequest ( $sql, array (
				$login, "local"
		) );
		if ($user->rowCount () == 1)
	...
	public function getUserFromIdUnit($id, $unit_id) {
		$sql = "SELECT name, firstname, id_responsible, id_unit FROM su_users WHERE id=?";
		if ($unit_id > 0) {
	...
	public function getLastConnection($id) {
		$sql = "select date_last_login from su_users where id=?";
		$user = $this->runRequest ( $sql, array (
				$id 
		) );
		if ($user->rowCount () == 1) {
	...
	private function updateUserActiveContract() {
		$sql = "select id, date_end_contract from su_users where is_active=1";
		$req = $this->runRequest ( $sql );
		$users = $req->fetchAll ();
	...
	private function updateUserActiveLastLogin($numberYear) {
		$sql = "select id, date_last_login from su_users where is_active=1";
		$req = $this->runRequest ( $sql );
		$users = $req->fetchAll ();
	...
	public function activate($userId) {
		...
		$sql = "update su_users set is_active=?, date_last_login=? where id=?";
		$this->runRequest ( $sql, array (
				1,
				$today,
				$userId 
	...
	public function getConventionList() {
		$sql = "select distinct convention from su_users order by convention";
		$req = $this->runRequest ( $sql );
		return $req->fetchAll ();
	}
	/**
	...
	public function getUserConvention($id) {
		$sql = "select convention from su_users where id=?";
		$req = $this->runRequest ( $sql, array (
				$id 
		) );
		$tmp = $req->fetch ();
	...
	public function exportResponsible($idType) {
		...
		if ($idType == 0) { // all
			$sql = "SELECT * FROM su_users WHERE id IN (SELECT id_users FROM su_responsibles) ORDER BY name ASC";
			$req = $this->runRequest ( $sql );
			$resps = $req->fetchAll ();
		} else if ($idType == 1) { // active
			$sql = "SELECT * FROM su_users WHERE id IN (SELECT id_users FROM su_responsibles) AND is_active=? ORDER BY name ASC";
			$req = $this->runRequest ( $sql, array(1));
			$resps = $req->fetchAll ();
			$typeResp = "actifs";
		} else if ($idType == 2) { // inactive
			$sql = "SELECT * FROM su_users WHERE id IN (SELECT id_users FROM su_responsibles) AND is_active=0 ORDER BY name ASC";
	...
	public function delete($id){
		$sql="DELETE FROM su_users WHERE id = ?";
		$req = $this->runRequest($sql, array($id));
	}
	public function setEndContract($login, $date_fin_contrat){
		$sql = "update su_users set date_end_contract=? where login=?";
		$this->runRequest ( $sql, array (
				$date_fin_contrat,
				$login
		) );
