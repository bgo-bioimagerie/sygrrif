 ...
 */
class SuPricing extends Model {
	
	public function createTable(){
		$sql = "CREATE TABLE IF NOT EXISTS `su_pricing` (
	...
	public function getPrices($sortEntry = 'id'){
		...
		$sql = "select * from su_pricing order by " . $sortEntry . " ASC;";
		$data = $this->runRequest($sql);
		return $data->fetchAll();
	}
	
	public function pricingsIDName(){
		$sql = "select id, tarif_name from su_pricing";
		$data = $this->runRequest($sql);
		return $data->fetchAll();
	}
	...
	public function getPricingId($name){
		$sql = "select * from su_pricing where tarif_name=?;";
	...
	public function editPricing($id, $name){
		$sql = "update su_pricing set tarif_name= ?
									  where id=?";
