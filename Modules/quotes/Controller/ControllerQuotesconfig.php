...
require_once 'Modules/quotes/Model/QoInstall.php';

/**
 * Configuation controller for the template module
 * @author sprigent
 */
class ControllerQuotesconfig extends ControllerSecureNav {
	...
	public function index() {
		...
		$status = $ModulesManagerModel->getDataMenusUserType("quotes");
		$menus[0] = array("name" => "quotes", "status" => $status);

		// install section
		$installquery = $this->request->getParameterNoException ( "installquery");
		if ($installquery == "yes"){
			try{
				$installModel = new QoInstall();
		...
		if ($setmenusquery == "yes"){
			...
			$ModulesManagerModel = new ModulesManager();
			$ModulesManagerModel->setDataMenu("quotes", "quotes", $menusStatus[0], "glyphicon glyphicon-credit-card");
