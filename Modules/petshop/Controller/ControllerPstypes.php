...
class ControllerPstypes extends ControllerSecureNav {
    ...
    public function index() {
        ...
        $table->addPrintButton("pstypes/index/");
    ...
    public function edit() {
        ...
        // form
        // build the form
        $form = new Form($this->request, "pstypes/edit");
        $form->setTitle(PsTranslator::Edit_Spice($lang));
        $form->addHidden("id", $unit["id"]);
        $form->addText("name", CoreTranslator::Name($lang), true, $unit["name"]);

        $form->setValidationButton(CoreTranslator::Ok($lang), "pstypes/edit");
        $form->setCancelButton(CoreTranslator::Cancel($lang), "pstypes");

        if ($form->check()) {
            // run the database query
            $this->model->set($form->getParameter("id"), $form->getParameter("name"));

            $this->redirect("pstypes");
        } else {
            // set the view
